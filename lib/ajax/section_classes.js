...
resource_class.prototype.init_buttons = function() {
    ...
    for (var x=0; x<buttons.length; x++) {
        ...
        } else if (buttons[x].className == 'editing_makepublic') {
            this.publicprivate = 1;
        } else if (buttons[x].className == 'editing_makeprivate') {
    ...
    if (this.publicprivate == 0) {
        var button = main.mk_button('a', main.portal.icons['publicprivate_public'], main.portal.strings['publicprivate_makeprivate']);
    } else if(this.publicprivate > 0) {
        var button = main.mk_button('a', main.portal.icons['publicprivate_private'], main.portal.strings['publicprivate_makepublic']);
    }
    YAHOO.util.Event.addListener(button, 'click', this.toggle_publicprivate, this, true);
    commandContainer.appendChild(button);
    this.viewButton = button;
...
resource_class.prototype.toggle_publicprivate = function(target, e, superficial, force) {
    
    var strpublic = main.portal.strings['publicprivate_makeprivate'];
    var strprivate = main.portal.strings['publicprivate_makepublic'];
    var strgrouping = main.portal.strings['publicprivate_groupingname'];
    
    var commands_node = null;
    var grouping_node = null;
    for(var i = 0,
            nodes = this.getEl().childNodes[0].childNodes,
            node = null;
            i < nodes.length,
            node = nodes[i];
            i++)
    {
        ...
        {
            case 'commands': commands_node = node; break;
            case 'groupinglabel': grouping_node = node; break;
        }
    }
    ...
    if (this.publicprivate == 0)
    {
        if(grouping_node != null)
        {
            grouping_node.parentNode.removeChild(grouping_node);
        }
        var new_ele = document.createElement('span');
        new_ele.className = 'groupinglabel';
        new_ele.innerHTML = '('+strgrouping+') ';
        YAHOO.util.Dom.insertBefore(new_ele, commands_node);
            
        this.viewButton.childNodes[0].src = this.viewButton.childNodes[0].src.replace(/public/i, 'private');
        this.viewButton.childNodes[0].alt = this.viewButton.childNodes[0].alt.replace(strpublic, strprivate);
        this.viewButton.title = this.viewButton.title.replace(strpublic, strprivate);
        this.publicprivate = 1;
        if (!superficial)
        {
            main.connect('POST', 'class=resource&field=private', null, 'id='+this.id);
        }
    }
    else
    {
        ...
        this.viewButton.childNodes[0].src = this.viewButton.childNodes[0].src.replace(/private/i, 'public');
        this.viewButton.childNodes[0].alt = this.viewButton.childNodes[0].alt.replace(strprivate, strpublic);
        this.viewButton.title = this.viewButton.title.replace(strprivate, strpublic);
        this.publicprivate = 0;

        if (!superficial)
        {
            main.connect('POST', 'class=resource&field=public', null, 'id='+this.id);
        }
    }
}
