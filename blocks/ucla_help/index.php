...
require_once(dirname(__FILE__) . '/config.php'); // helpblock config settings
...
if ($fromform = $mform->get_data()) {
    ...
    if ('email' == $block_ucla_help->config->send_to) {
        ...
        // now add support contact email address (if available)
 ...
 * level. First looks for the point of contact in the local configuration file,
 * if not found, then looks in the ucla help block table that maps context to a
 * contact person.
 * 
 * @global type $ucla_help_poc
 * 
 * @param object $context_id        Current context object
 */
function get_support_contact($cur_context)
{
    global $ucla_help_poc;
    $ret_val = null;
    
    // get list of contexts to check
    $context_ids = array_merge((array) $cur_context->id, (array) get_parent_contexts($cur_context));
    ...
    foreach ((array) $context_ids as $context_id) {
        ...
        print_object($context_name);
        
        if (!empty($ucla_help_poc[$context_name])) {
            $ret_val = $ucla_help_poc[$context_name];
            break;
        }        
    }   
    
    // todo: Merge $ucla_help_poc with database table for user created context mapping
    
    // todo: should raise error if ret_val is null
